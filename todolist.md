...
Search engine: {
 ...
 - [?] Add Pop up display for previewing the image/website by hover to AppID number.
 - [+] Add open in new tab on the AppID number (AppID has link to Steam Store), hover to show tooltip open in new tab.
}
},
...
Games List: {
 ...
 - [WIP] AVOID DUPLICATE AppID when importing the list.
},
...
Export/Import: {
  ...
  - [+] Add confirmation dialog when export/import the list.
...
Generate Button: {
  ...
  - [WIP] Generate into filled text `.bat` file (Windows). Using `ECHO {AppID}>{Priority}.txt`. 
  - [WIP] Generate into filled text `.txt` files
}
